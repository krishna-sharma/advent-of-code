...
let i;
let sumLast4 = 0;
puzzleInput.forEach((inputLine) => {
  const first10 = inputLine.split(" | ")[0].trim().split(" ");

  // digit 1
  const one = new Set(
    first10.filter((outputNum) => outputNum.trim().length === 2)[0].split("")
  );
  // digit 4
  const four = new Set(
    first10.filter((outputNum) => outputNum.trim().length === 4)[0].split("")
  );
  // digit 7
  const seven = new Set(
    first10.filter((outputNum) => outputNum.trim().length === 3)[0].split("")
  );
  // digit 8
  const eight = new Set(
    first10.filter((outputNum) => outputNum.trim().length === 7)[0].split("")
  );
  // digit 3
  const three = first10
    .filter((outputNum) => outputNum.trim().length === 5)
    .map((candidate) => new Set(candidate.split("")))
    .map((candidate) => {
      for (cfValue of one.values()) {
        candidate.add(cfValue);
      }
      return candidate;
    })
    .filter((candidate) => {
      let segments = 0;
      for (candidateValue of candidate.values()) segments += 1;
      return segments === 5;
    })[0];
  // digit 6
  const six = first10
    .filter((outputNum) => outputNum.trim().length === 6)
    .map((candidate) => new Set(candidate.split("")))
    .filter((candidate) => {
      const temp = new Set(candidate.values());
      for (oneValue of one.values()) {
        temp.add(oneValue);
      }
      return Array.from(temp).length === 7;
    })[0];
  // digit 9
  const nine = first10
    .filter((outputNum) => outputNum.trim().length === 6)
  ...
  const zero = first10
    ...
    .map((candidate) => new Set(candidate.split("")))
    .filter((candidate) => {
      let sixString = Array.from(six).sort().join("");
      let nineString = Array.from(nine).sort().join("");
      let zeroString = Array.from(candidate).sort().join("");
      return zeroString !== sixString && zeroString !== nineString;
    })[0];
  // digit 5
  const five = first10
    .filter((outputNum) => outputNum.trim().length === 5)
    ...
    .filter((candidate) => {
      ...
      return Array.from(temp).sort().join("") === nineString;
    })[0];
  // digit 2
  const two = first10
    .filter((outputNum) => outputNum.trim().length === 5)
  ...
  const numStrings = [
    Array.from(zero).sort().join(""),
    Array.from(one).sort().join(""),
    Array.from(two).sort().join(""),
    Array.from(three).sort().join(""),
    Array.from(four).sort().join(""),
    Array.from(five).sort().join(""),
    Array.from(six).sort().join(""),
    Array.from(seven).sort().join(""),
    Array.from(eight).sort().join(""),
    Array.from(nine).sort().join(""),
  ];

  const last4 = inputLine
    .split(" | ")[1]
    .trim()
    .split(" ")
    .map((num) => numStrings.indexOf(num.split("").sort().join("")).toString())
    .join("");

  console.log(last4);
  sumLast4 += parseInt(last4);
});

console.log({ sumLast4 });
